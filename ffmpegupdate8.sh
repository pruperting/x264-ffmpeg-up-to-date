...
cd ffmpeg 2>> $LOG >> $LOG
./configure --enable-gpl --enable-libfaac --enable-libmp3lame --enable-libopencore-amrnb --enable-libopencore-amrwb --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-nonfree --enable-postproc --enable-version3 --enable-x11grab 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
...
make distclean 2>> $LOG >> $LOG
git pull 2>> $LOG >> $LOG
./configure --enable-gpl --enable-libfaac --enable-libmp3lame --enable-libopencore-amrnb --enable-libopencore-amrwb --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-nonfree --enable-postproc --enable-version3 --enable-x11grab $FFMPEGCONFIGURE 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=ffmpeg --pkgversion="5:$(date +%Y%m%d%H%M)-git" --backup=no --deldoc=yes --fstrans=no --default 2>> $LOG >> $LOG
hash x264 ffmpeg ffplay ffprobe 2>> $LOG >> $LOG
}
...
lucid_dep ()
{
apt-get -y remove ffmpeg x264 libx264-dev libmp3lame-dev 2>> $LOG >> $LOG
apt-get update 2>> $LOG >> $LOG
...
./configure --enable-static 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=x264 --default --pkgversion=""3:$(./version.sh | awk -F'[" ]' '/POINT/{print $4"+git"$5}')"" --backup=no --deldoc=yes 2>> $LOG >> $LOG
}
...
make distclean 2>> $LOG >> $LOG
git pull 2>> $LOG >> $LOG
./configure --enable-gpl --enable-version3 --enable-nonfree --enable-postproc --enable-libfaac --enable-libopencore-amrnb --enable-libopencore-amrwb --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-x11grab --enable-libmp3lame $FFMPEGCONFIGURE 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=ffmpeg --pkgversion="5:$(./version.sh)" --backup=no --deldoc=yes --default 2>> $LOG >> $LOG
hash x264 ffmpeg ffplay ffprobe 2>> $LOG >> $LOG
}
...
hardy_dep ()
{
apt-get -y remove ffmpeg x264 libx264-dev liblame-dev 2>> $LOG >> $LOG
apt-get -y update 2>> $LOG >> $LOG
apt-get -y install nasm libogg-dev build-essential git-core checkinstall texi2html libfaac-dev libsdl1.2-dev libvorbis-dev libx11-dev libxext-dev libxfixes-dev libxvidcore4-dev zlib1g-dev 2>> $LOG >> $LOG
cd $INSTALL 2>> $LOG >> $LOG
wget http://www.tortall.net/projects/yasm/releases/yasm-1.1.0.tar.gz 2>> $LOG >> $LOG
tar xzvf yasm-1.1.0.tar.gz 2>> $LOG >> $LOG
cd yasm-1.1.0 2>> $LOG >> $LOG
./configure 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=yasm --pkgversion="1.1.0" --backup=no --deldoc=yes --default 2>> $LOG >> $LOG
cd $INSTALL 2>> $LOG >> $LOG
wget http://downloads.sourceforge.net/project/lame/lame/3.98.4/lame-3.98.4.tar.gz 2>> $LOG >> $LOG
tar xzvf lame-3.98.4.tar.gz 2>> $LOG >> $LOG
cd lame-3.98.4 2>> $LOG >> $LOG
./configure --enable-nasm --disable-shared 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=lame-ffmpeg --pkgversion="3.98.4" --backup=no --deldoc=yes --fstrans=no --default 2>> $LOG >> $LOG

wget http://downloads.xiph.org/releases/theora/libtheora-1.1.1.tar.gz 2>> $LOG >> $LOG
tar xzvf libtheora-1.1.1.tar.gz 2>> $LOG >> $LOG
cd libtheora-1.1.1 2>> $LOG >> $LOG
./configure --disable-shared 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=libtheora --pkgversion="1.1.1" --backup=no --deldoc=yes --fstrans=no --default 2>> $LOG >> $LOG
...
./configure --enable-static 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=x264 --pkgversion="1:0.svn$(date +%Y%m%d)-0.0ubuntu1" --backup=no --deldoc=yes --default 2>> $LOG >> $LOG
}
...
git clone git://git.videolan.org/ffmpeg 2>> $LOG >> $LOG
cd ffmpeg 2>> $LOG >> $LOG
./configure --enable-gpl --enable-version3 --enable-nonfree --enable-postproc --enable-libfaac --enable-libmp3lame --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-x11grab 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=ffmpeg --pkgversion="4:git-$(date +%Y%m%d)" --backup=no --deldoc=yes --default 2>> $LOG >> $LOG
...
make distclean 2>> $LOG >> $LOG
git pull 2>> $LOG >> $LOG
./configure --enable-gpl --enable-version3 --enable-nonfree --enable-postproc --enable-libfaac --enable-libmp3lame --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libxvid --enable-x11grab $FFMPEGCONFIGURE 2>> $LOG >> $LOG
make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG
checkinstall --pkgname=ffmpeg --pkgversion="4:git-$(date +%Y%m%d)" --backup=no --deldoc=yes --default 2>> $LOG >> $LOG
hash x264 ffmpeg ffplay 2>> $LOG >> $LOG
}
...
OKDISTRO="hardy lucid maverick natty"
